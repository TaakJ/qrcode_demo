...
def broadcast_realtime(self):
    try:
        ...
        message = json.dumps([{'feed': qs_json.feed, 
                            ...
                            #    'dual_date': qs_json.dual_date,
                                'vote': qs_json.vote_star} for qs_json in qs])    
...
def push_message_job(self):
    ...
    for instance in model_profile:
        ...
        if schedule_plan != 0:
            vote_percent = instance['vote_percent'] - instance['avg_vote']
            vote_star = int((7 * instance['vote_percent']) / 100)
